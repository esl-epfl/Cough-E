...
def run_config(cfg):
    ...
    test_subj_ids = os.listdir(cfg.test_data_folder)
    print("Loaded data from {0} subjects".format(len(test_subj_ids)))
    ...
    X_train, y_t, subjects_train, feat_names,  trial_logs, mov_logs, noise_logs = get_data_matrices(subj_ids, cfg.data_folder, features_folder, cfg.experiment_conditions.conditions_train, n_audio_feat,n_imu_feat,cfg.signals.in_mic_sel,cfg.signals.out_mic_sel,cfg.signals.imu_sel, cfg.signals.biodata_sel)
    y_train = (y_t>0)

    X_test, y_t, subjects_test, _,  _, _, _ = get_data_matrices(test_subj_ids, cfg.test_data_folder, features_folder, cfg.experiment_conditions.conditions_test, n_audio_feat,n_imu_feat,cfg.signals.in_mic_sel,cfg.signals.out_mic_sel,cfg.signals.imu_sel, cfg.signals.biodata_sel)
    y_test = (y_t>0)

    # Set up CV for choices of parameters
    np.random.seed(0)
    scoring_method = "average_precision"
    cv = GroupKFold(n_splits=cfg.ml.n_splits_inner)
    ...
    if cfg.ml.steps.select_model:
        ...
        for i, mn in enumerate(cfg.ml.model_names):
            mean_score, std_score, _ = train_model_untuned(X_train, y_train, subjects_train, cv, mn, scoring_method, scaling=cfg.ml.feature_scaling, smote=cfg.ml.smote)
        ...
        df_results["std"] = stds
        df_results.to_csv(output_folder + "/model_comparison_results.csv")
    ...
    if cfg.ml.tune_model_hyps:
        ...
        tuned_result , model_hyps = tuneHyperparameters(X_train, y_train, subjects_train, cv, [chosen_model], scoring_method)
    ...
    if cfg.ml.steps.rfecv:
        ...
        rfe = RFECV(estimator=model, step=cfg.ml.step_size, cv=cv, scoring=scoring_method)
        ...
        df_results.to_csv(output_folder + "/rfecv_results.csv")
    ...
    if cfg.ml.steps.feature_selection: 
        ...
        if ~(cfg.ml.steps.rfecv) | (cfg.ml.overwrite_n_features_opt):
            ...
            for i, (train_index_inner, test_index_inner) in enumerate(cv.split(X_train, y_train, subjects_train)):
        ...
        pickle.dump(feat_select, open(output_folder + '/feat_select.pkl', 'wb'))
    # else:
    ...
    if cfg.ml.steps.train:
        ...
        # Train model
        if cfg.ml.tune_model_hyps:
            _ , model_hyps = tuneHyperparameters(X_train, y_train, subjects_train, cv, [chosen_model], scoring_method)
            model = generate_pipeline(chosen_model, model_hyps[0], scaling=cfg.ml.feature_scaling, smote=cfg.ml.smote)
        else:
            model = generate_pipeline(chosen_model, scaling=cfg.ml.feature_scaling, smote=cfg.ml.smote)
        ...
        # Save model
        pickle.dump(model, open(output_folder + '/model.pkl', 'wb'))
    else:
        model_folder = cfg.ml.model_feat_folder
    ...
    if cfg.ml.steps.test:
        ...
        tn, fp, fneg, tp, se, sp, acc, pr, f1, auc, npv, fphr = report_sota_metrics(y_test, test_prob, cfg.decision_threshold, cfg.signals.window_len)
        with open(output_folder + "/traditional_metrics.txt", 'a') as file:
            file.write("True positivies (TP):{0}".format(tp))
            file.write("True negatives (TN):{0}".format(tn))
            file.write("False positivies (FP):{0}".format(fp))
            file.write("False negatives (FN):{0}".format(fneg))
            file.write("Sensitivity (SE):{0}".format(se))
            file.write("Precision (PR):{0}".format(pr))
            file.write("Specificity (SP):{0}".format(sp))
            file.write("Accuracy (ACC):{0}".format(acc))
            file.write("F-1 Score (F1):{0}".format(f1))
            file.write("Negative predictive value (NPV):{0}".format(npv))
            file.write("False positives per hour (FPHR):{0}".format(fphr))
            file.write("ROC-AUC score:{0}".format(auc))
        ...
        for test_subj_id in test_subj_ids:        
                ...
                for mov in cfg.experiment_conditions.conditions_test.movements:
                    for noise in cfg.experiment_conditions.conditions_test.noises:
                        for sound in cfg.experiment_conditions.conditions_test.sounds:
                            path = cfg.test_data_folder + test_subj_id + '/trial_' + trial + '/mov_' + mov + '/background_noise_' + noise + '/' + sound
                            if os.path.exists(path):
                                ...
                                if sum(required_biodata)>0:
                                    fn = cfg.test_data_folder + test_subj_id + '/biodata.json'
                                    with open(fn, 'rb') as f:
                                        bio = json.load(f)
                                    ...
                                    extra_features = None
                                air, skin = load_audio(cfg.test_data_folder, test_subj_id,  cfg.signals.fs_audio, trial, mov, noise, sound)
                                imu = load_imu(cfg.test_data_folder, test_subj_id, trial, mov, noise, sound)
                                ...
                                #performance metrics
                                if cfg.signals.out_mic_sel: # Audio data included --> use postprocessing
