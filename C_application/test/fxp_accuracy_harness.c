...
int main(void)
{
    ...
    for (int w = 0; w < n_wins; w++) {
        ...
        for (int ax = 0; ax < 6; ax++) {
            ...
            uq11_16_t xv = fxp_get_rms_raw(raw_fxp[ax], WIN_LEN);
        ...
        for (int ax = 0; ax < 6; ax++) {
            ...
            uq3_23_t xv = fxp_get_line_length_raw(raw_fxp[ax], WIN_LEN);
            emit(w, "get_line_length", ax_names[ax], fv, FXP_TO_FLOAT(xv, 23));
        }
        {
            float fv = get_line_length(l2g_f, WIN_LEN);
            uq2_9_t xv = fxp_get_line_length_l2g(l2g_fxp, WIN_LEN);
        ...
        for (int ax = 0; ax < 6; ax++) {
            ...
            q6_30_t xv = fxp_get_kurtosis_raw(raw_fxp[ax], WIN_LEN);
