...
static inline uint32_t fxp_ll_raw_diff_to_accum(int32_t diff)
    { return (uint32_t)fxp_abs_s32(diff); }   // no early shift
...
{
    return (q6_30_t)((sum_x4 << 24) / (denom >> 30))
           - FXP_KURT_FISHER_Q6_30;
}
...
static inline uint64_t fxp_rms_raw_sq_to_accum(uint32_t sq)
    { return (uint64_t)sq; }   // no early shift; accumulate full Q22.10 values
...
#define FXP_DEFINE_GET_KURTOSIS_RAW()                                                 \
    q6_30_t fxp_get_kurtosis_raw(const q11_5_t *sig, int16_t len){                  \
        ...
        int32_t _sum_mean = 0;                                                        \
        for(int16_t _i = 0; _i < len; _i++) _sum_mean += (int32_t)sig[_i];          \
        ...
        if(_denom == 0) return 0;   /* guard: std==0, undefined */                   \
...
#define FXP_DEFINE_L2_NORM_ACCEL()                                                    \
    uq10_6_t fxp_L2_norm_accel(float ax, float ay, float az){                       \
        ...
        uint32_t _sum = (uint32_t)fxp_mul_s32(_ax, _ax)                             \
                      + (uint32_t)fxp_mul_s32(_ay, _ay)                              \
                      + (uint32_t)fxp_mul_s32(_az, _az);                             \
...
#define FXP_DEFINE_L2_NORM_GYRO()                                                     \
    uq5_11_t fxp_L2_norm_gyro(float gx, float gy, float gz){                        \
        ...
        uint32_t _sum = (uint32_t)fxp_mul_s32(_gx, _gx)                             \
                      + (uint32_t)fxp_mul_s32(_gy, _gy)                              \
                      + (uint32_t)fxp_mul_s32(_gz, _gz);                             \
...
#define FXP_DEFINE_AZC_POLYGONAL_APPROX(SUFFIX, sample_t)                             \
    static int16_t *fxp_polygonal_approx_##SUFFIX(const sample_t *sig, int16_t len,  \
        ...
        _stk[0].first = 0; _stk[0].last = len - 1;                                   \
        int16_t _next = 0;                                                            \
        while(_next >= 0){                                                            \
            ...
            if(_md > eps_fxp){                                                        \
                _stk[_next+1].first = _f;   _stk[_next+1].last = _mid;               \
                _stk[_next+2].first = _mid; _stk[_next+2].last = _l;                 \
                _next += 2;                                                           \
            } else {                                                                  \
                int16_t _af = 1, _al = 1;                                             \
