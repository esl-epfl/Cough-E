...
//
// Replaces the #ifdef FXP_MODE / void*+switch pattern with tagged signal
// structs and _Generic macros.  Call sites construct the appropriate struct
// type; the dispatch macros route to the correct kernel automatically.
//
// Float mode:  imu_sig_float_t  ->  float kernels (get_rms, get_line_length, …)
...
//
// Each struct wraps a typed pointer + length.  The struct type is what
// _Generic dispatches on — the tag carries no runtime overhead.
// =============================================================================

// Float-mode signal (used when FXP_MODE is not defined, or as fallback)
...
//
// _Generic requires each arm to be a function (not a macro).  These wrappers
// accept the tagged struct, call the underlying kernel, and convert the result
// to float (the model's feature vector is always float).
//
...
static inline float imu_dispatch_line_length_raw(imu_sig_raw_t s) {
    return FXP_TO_FLOAT(fxp_get_line_length_raw(s.data, s.len), 23);
}
...
// RAW: max and crest factor not used in the model, return 0
...
static inline float imu_dispatch_line_length_l2a(imu_sig_l2a_t s) {
    (void)s; return 0.0f;  // L2_A: no line length in model
}
...
static inline float imu_dispatch_kurtosis_l2a(imu_sig_l2a_t s) {
    (void)s; return 0.0f;  // L2_A: kurtosis not in model
}
...
static inline float imu_dispatch_max_l2a(imu_sig_l2a_t s) {
    (void)s; return 0.0f;  // not used for L2_A
}
...
static inline float imu_dispatch_kurtosis_l2g(imu_sig_l2g_t s) {
    (void)s; return 0.0f;  // L2_G: kurtosis not in model
}
...
//
// Each macro selects the correct wrapper based on the struct type of `sig`.
// In float-only mode, there is only one arm.  In FXP_MODE, four arms cover
// float + the three FxP signal types.
//
...
// =============================================================================
// Section 4 — Unified imu_signal_features (type-generic)
//
// This macro-function replaces both imu_signal_features() (float) and
// imu_signal_features_fxp() (void*+switch).  It works with ANY signal struct
// type — _Generic resolves the correct kernel at compile time.
//
// The features_selector offsets and feature enum values are identical
// regardless of signal type, so the control flow is shared.
// =============================================================================
